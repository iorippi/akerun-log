...
class AkerunLog {
	...
	public function __construct($options) {
		...
		if (!count($akerun_log_cache) || $akerun_log_cache_expire < $log_til) {
			...
			$log_json = file_get_contents($log_api_url);
		...
		if ($log[success])
			$this->akerun_json_log = "Success";
		elseif (!$log[success])
			$this->akerun_json_log = "API retrieval failed [success: ".$log[success]."].";
		elseif ($log == null)
			$this->akerun_json_log = "Log is null.";
		else
			$this->akerun_json_log = "Unknown Error";
...
class AkerunLogByUsers extends AkerunLog {
	...
	public function __construct($options) {
		...
		if ($this->akerun_json_log !== "Success")
			return null;
